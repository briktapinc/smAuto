...
import shutil
import random
from utils import removeTags, getFilenameOfLine, getTopic, capitalize, lineBillboardStem
...
def switchLines(first):
    ...
    if not first: # We don't want to save any images on the very first initialization of the program.
        if not os.path.isdir(B_FOLDER):
            os.mkdir(B_FOLDER)
        # Short b001.png names — topic text as a filename hits Windows MAX_PATH.
        pygame.image.save(canvassy, B_FOLDER+"/"+lineBillboardStem(LINE_ON + 1)+".png")
    while LINE_ON < len(lines) and (
        os.path.exists(B_FOLDER+"/"+lineBillboardStem(LINE_ON + 1)+".png")
        or os.path.exists(B_FOLDER+"/"+getFilenameOfLine(lines[LINE_ON])+".png")
    ): # If you've already drawn that image, skip it and move on to the next one.
        LINE_ON += 1
